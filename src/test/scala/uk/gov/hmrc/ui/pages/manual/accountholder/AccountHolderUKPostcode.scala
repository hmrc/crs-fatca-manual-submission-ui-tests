...
object AccountHolderUKPostcode extends BasePage {
  ...
  val linkForManualEntry: By = By.linkText("Or enter the address manually")

  def checkPage(): this.type = {
    onPage(pageUrl)
    checkH1("What is the postcode for")
    this
  }

  def enterPostcodeAndContinueForAddress(postCode: String): this.type = {
    onPage(pageUrl)
