...
trait BasePage extends BrowserDriver with Matchers with IdGenerators with PageObject {
  ...
  val currencySelectId: By     = By.id("currency-select")
  ...
  def selectCurrency(currency: String): this.type = {
    onPage(pageUrl)
    val js = Driver.instance.asInstanceOf[JavascriptExecutor]
    js.executeScript("document.getElementById('currency-select').style.display = 'block';")
    new Select(Driver.instance.findElement(currencySelectId)).selectByVisibleText(currency)
    this
  }
